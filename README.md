...
---
### ReadFi MVP — 已完成智能合約總覽
| 類別                     | 合約名稱                                                  | 檔案位置                                                | 標準                      | 功能摘要                                                                          | 狀態                            |
| ---------------------- | ----------------------------------------------------- | --------------------------------------------------- | ----------------------- | ----------------------------------------------------------------------------- | ----------------------------- |
|  **平台代幣 (ERC20)**    | **READToken**                                         | `contracts/contracts/READToken.sol`                 | ERC-20                  | 平台原生代幣，用於：購書支付、DAO 投票、分潤與激勵。                                                  | ✅ 已完成、可測試運作                   |
|  **多書籍資產 (ERC1155)** | **ReadFiBooks1155**                                   | `contracts/contracts/ReadFiBooks1155.sol`           | ERC-1155                | 每本書為一個 SKU，支援批量鑄造、版本控制、URI metadata；包含受規範 SKU 控制欄位。                           | ✅ 已完成並整合合規模組                  |
|  **鑄造閘道 / 中繼層**      | **ReadFiMintGateway**                                 | `contracts/contracts/ReadFiMintGateway.sol`         | Custom                  | 前端與後端的橋樑：接收簽章 + metadata → 呼叫 Books1155 鑄造；支援合規檢查與自動分潤。                       | 🚧 測試中                        |
|  **合規管理核心**          | **ReadFiComplianceManagerV2**                         | `contracts/contracts/ReadFiComplianceManagerV2.sol` | ERC-3643 模擬層            | 模擬 ERC-3643 TransferManager / IdentityRegistry。負責判定錢包是否通過 Self VC 或 Oasis 驗證。 | ✅ 已部署於 Zircuit Testnet（Proxy） |
|  **TEE 驗證閘道 (隱私層)**  | **ComplianceGateway**                                 | `contracts/contracts/ComplianceGateway.sol`         | Custom (Oasis Sapphire) | 部署於 Oasis Sapphire TEE，負責驗證 VC / Proof、檢查黑名單、年齡、地區，並返回布林結果給 Zircuit。          | 🚧 開發中                        |
|  **升級管理腳本**          | `scripts/upgrade/deployOrUpgradeComplianceManager.ts` | Hardhat-Upgrades                                    | 自動判斷 Proxy 狀態並執行升級或新部署。 | ✅ 已測試可用                                                                       |                               |
...
| 階段      | 目標                               | 時間      | 狀態     |
| ------- | -------------------------------- | ------- | ------ |
| Phase 1 | 完成合約、Oasis 驗證邏輯        | 2025 Q1 | ✅ Done |
